    ...
    <div className='my-16 flex w-full flex-col items-center justify-center sm:my-20'>
        ...
        {button && (
            <Link href={button.href}>
                <Button className='paragraph-medium mt-5 min-h-[46px] cursor-pointer rounded-lg bg-primary-500 px-4 py-3 text-light-900 hover:bg-primary-500'>
                    {button.text}
                </Button>
            </Link>
        )}
...
export default function DataRender<T,>({
    success, error, data, empty = DEFAULT_EMPTY, render
}: Props<T>) {
    if (!success) {
        return (
            <StateSkeleton
